...
//!
//! Registry version 1 deliberately stamps only exact direct streams. Logical
//! relationship permissions are recorded as authorization, but no relationship
//! transform is attempted by this first size-attempt mechanism.
...
impl AuraPlanV2 {
    ...
    pub fn validate(&self, schema: &SchemaDescriptor) -> Result<()> {
        ...
        if self.plan_version != AURA_PLAN_V2_VERSION
            || self.registry_version != AURA_PLAN_V2_REGISTRY_VERSION
        ...
        for descriptor in &self.streams {
            if descriptor.op != AURA_PLAN_V2_DIRECT_OP || !descriptor.dependencies.is_empty() {
    ...
    pub fn encode(&self, schema: &SchemaDescriptor) -> Result<Vec<u8>> {
        ...
        let length = HEADER_BYTES
            .checked_add(
                self.streams
                    .len()
                    .checked_mul(STREAM_PREFIX_BYTES)
                    .ok_or(AuraError::InvalidValue("aura plan v2 length"))?,
            )
        ...
        let hash = plan_hash_bytes(&bytes)?;
    ...
    pub fn decode(schema: &SchemaDescriptor, bytes: &[u8]) -> Result<Self> {
        ...
        if plan_hash_bytes(&bytes[..hash_start])? != bytes[hash_start..] {
        ...
        if stream_count > MAX_AURA_PLAN_V2_STREAMS
            ...
            || stream_count > reader.remaining() / STREAM_PREFIX_BYTES
        {
...
fn plan_hash_bytes(bytes: &[u8]) -> Result<[u8; 32]> {
    let len =
