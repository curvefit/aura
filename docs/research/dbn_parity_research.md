...
|---|---|---|---|---|
| End-to-end representation | One DBN stream/file layout is usable for storage, transport, and in-memory record handling. | Aura has three explicit tiers: `.aura` ingest, `.aura0` compact, `.aura1` fixed-width replay. `.aura1` is now documented as the disk/in-memory/live hot-path candidate. | Aura intentionally does not collapse ingest, cold storage, and replay into one binary representation. | Keep `.aura1` as the replay/transport candidate and keep `.aura`/`.aura0` roles explicit. |
| Same code historical/live | Historical replay and live callbacks both dispatch `DBNRecord`-like events. | Phase 3 added `AuraEventSource`, `AuraEventBatch`, `AuraFileSource`, `AuraMemorySource`, and `AuraLiveSource<R>` for Aura1 fixed batches. | The common source interface is Aura1-only; `.aura`/`.aura0` must convert or expand before shared replay. | Extend the source family only when a non-Aura1 source can emit the same fixed-batch semantics honestly. |
| Zero-copy | Fixed structs allow clients to borrow/interpret records without conversion layers. | Aura1 has `Aura1FixedBatchView`, `Aura1RowView`, selected row views, file-range stats, order-book delta batches, and event sources over borrowed fixed batches. | Generic `next_batch` materializes `AuraRecordBatch`; `Read` sources copy full bytes; file-backed batches currently allocate per chunk rather than mmap. | Keep borrowed Aura1 batch APIs as the zero-copy surface; document allocation boundaries. |
...
| Fixed lengths and offsets | DBN records use a common header and fixed schema structs with predictable offsets. | Aura1 rows are fixed-width, schema-index ordered, little-endian, and driven by `CompiledAuraPlan`; source consumers can inspect the plan/record width. | Aura does not yet define transport framing or schema negotiation beyond supplying schema/plan out of band for live streams. | Treat Aura1 fixed batches as the hot replay/transport candidate and add framing only when needed. |
